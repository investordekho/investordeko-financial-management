@php
    $isSubscribed = isset($subscriber) && $subscriber->is_subscribed ?? false;
@endphp
   <!-- check print $userAccess array -->
@php
    /*
    echo '<pre>';
    print_r($userAccess);
    ...
    <div class="container mt-5">
        <div class="row">
            @foreach ($investees->take($visibleInvesteeCount) as $investee)
               ...
               /* echo '<pre>';
                    echo  "Investee ID: {$investee->id}\n";
                    echo  "Is Subscribed: " . ($isSubscribed ? 'true' : 'false') . "\n";
                    echo  "Is Approved: " . ($approved ? 'true' : 'false') . "\n";
                    echo  "Blur Class Applied: " . ($blurClass ? 'true' : 'false') . "\n";
                echo '</pre>'; */
                @endphp
                @if( $blurClass == '')
                ...
                <div class="col-md-12 mb-4">
                    <div class="investee-card p-4">
                        <div class="row align-items-center">
                            ...
                            <div class="col-md-4 text-end {{ $blurClass }}">
                                <!-- @if($isSubscribed)
                                    <a href="{{ route('investordashboard.investeelistdetail', ['id' => $investee->id]) }}" 
                                    class="btn btn-primary btn-sm">🔍 View Profile</a>
                                @else
                        ...
                        <div class="row text-center">
                            <div class="col-md-4">
                                <p class="info-label"><i class="bi bi-briefcase-fill text-info"></i> Business Type</p>
                                <!-- <p class="info-value {{ $blurClass }}">{{ $investee->nature_of_business ?? 'N/A' }}</p> -->
                                                          <p class="info-value {{ $blurClass }}">nature_of_business</p>
                            </div>
                            <div class="col-md-4">
                                <p class="info-label"><i class="bi bi-calendar-event text-warning"></i> Incorporated</p>
                                <!-- <p class="info-value {{ $blurClass }}">{{ $investee->incorporated_in ?? 'N/A' }}</p> -->
                                          <p class="info-value {{ $blurClass }}">incorporated_in</p>
                            </div>
                            <div class="col-md-4">
                                <p class="info-label"><i class="bi bi-cash-stack text-success"></i> Fund Usage</p>
                                <!-- <p class="info-value {{ $blurClass }}">{{ $investee->fundRequirements->first()->usage ?? 'N/A' }}</p> -->
                                <p class="info-value {{ $blurClass }}">fundRequirements</p>
                            </div>
                        </div>
                                ...
                                @if ($investee->previousRounds->count())
                                    @foreach ($investee->previousRounds as $round)
                                        <p class="investment-details {{ $blurClass }}">
                                            ...
                                            &nbsp;|&nbsp;
                                            <strong>Amount Raised:</strong> ₹ amount_raised Cr  
                                            &nbsp;|&nbsp;
                                            <strong>Valuation:</strong> ₹ valuation Cr
                                        </p>
                                    @endforeach
                                @else
                                    <p class="text-muted">No previous investments found.</p>
...
<style>
    ...
    .locked-content { filter: blur(5px); opacity: 0.6; user-select: none; pointer-events: none; }
